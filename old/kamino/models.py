...
class Population(models.Model):
    ...
    simulation = models.ForeignKey(Simulation, on_delete=models.CASCADE, null=False, related_name='populations')
    name = models.CharField(max_length=240)

class RagarajaInstructions(models.Model):

    simulation = models.ForeignKey(Simulation, on_delete=models.CASCADE, null=True, related_name='ragaraja_instructions')
    instruction = models.CharField(
        max_length=3,
        # choices=[(inst, inst) for inst in list(ragaraja.ragaraja)]
    )

class Location(models.Model):

    population = models.ForeignKey(Population, on_delete=models.CASCADE, null=False, related_name='locations')
